...
import { processDueBlastCampaigns } from "@/lib/blast";
...
export async function POST(request: NextRequest) {
  ...
  if (businesses.length === 0) {
    return NextResponse.json(
      { ok: false, reason: "Belum ada business yang terdaftar" },
      { status: 503 }
    );
  }
  ...
  if (!authorized) {
    return NextResponse.json(
      { ok: false, reason: "Invalid scheduler secret" },
      { status: 401 }
    );
  }
  ...
  for (const business of businesses) {
    try {
      const result = await processDueBlastCampaigns(business.id);
      results.push({
