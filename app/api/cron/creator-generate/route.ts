...
import { processDueCreatorDraftGenerationsForBusiness } from "@/lib/creator";
...
export async function POST(request: NextRequest) {
  // The scheduler secret is per-business. We accept the request if it matches at
  // least one business's schedulerSecret, then iterate every business that has
  // creatorGenerationEnabled and run their due slots independently.
  ...
  for (const business of businesses) {
    try {
      const result = await processDueCreatorDraftGenerationsForBusiness(business.id);
